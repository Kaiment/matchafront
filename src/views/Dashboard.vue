      ...
      .component.columns.is-multiline.is-centered
          .filters.column.is-8
              ...
              .field
                  ...
                  vue_slider(v-model='pop_interval', :min=-10000, :max=10000, :tooltip='false')
              .field
                  label Maximum {{ max_dist }}km away
                  vue_slider(v-model='max_dist', :min=0, :max=100, :tooltip='false')
          .sort.column.is-4
              label Sort by :
...
export default {
  ...
  methods: {
    ...
    test () {
      this.$http.post('http://10.4.3.5:8008/test', { foo: 'bar' }).then(res => {
        console.log(res);
      })
    }
  }
